root@gardener-local-control-plane:/# cat /usr/local/bin/entrypoint
#!/bin/bash
...
fix_cgroup() {
  # $$$$$$$ D060239: that's the only thing this script does: simply detects cgroupsv2 on Gardenlinux and returns
  # You can see the logs of the entrypoint script as logs on the machine-pod
  if [[ -f "/sys/fs/cgroup/cgroup.controllers" ]]; then
  ...
  # TODO: D060239: I think this is exactly what I need as privileged pods in Kubernetes on cgroupsv2 do not get a cgroup namespace :(
  # that behaviour is different from docker or podman (where kindest-node is supposed to run directly as a container)
